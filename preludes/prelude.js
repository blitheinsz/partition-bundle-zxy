  ...
  if (!module) {
    ...
    if (modules[id]) {
      var _req = function(_id) {
         var v = modules[id].deps[_id];
         if (!v) {
           return __require(_id).exports;
         }
      ...
      if (window.require) {
         module.exports = window.require(id, true);
      } else {
...
function loadScriptFile(file) {
  var fileUrl;
  if(loadjs.beginLoadFun) loadjs.beginLoadFun(file);
  if (loadjs.suffix)
      fileUrl = loadjs.url + file.substring(0, file.indexOf('.js')) + loadjs.suffix + '.js';
  else
      fileUrl = loadjs.url + file;
  loadScript(fileUrl, function(err) {
    var cbs = loading[file];
    if (!err) {
      loading[file] = null;
      loaded.push(file);
    }
    if(loadjs.endLoadFun) loadjs.endLoadFun(file);
...
loadjs.getDependLoadFile = function(file) {
  var dependFile = loadjs.files[loadjs.map[file][0]],
      fileUrl;
  if (loadjs.suffix) {
    fileUrl = loadjs.url + dependFile.substring(0, dependFile.indexOf('.js')) + loadjs.suffix + '.js';
  }
  else {
    fileUrl = loadjs.url + dependFile;
  }
  return fileUrl;
};
